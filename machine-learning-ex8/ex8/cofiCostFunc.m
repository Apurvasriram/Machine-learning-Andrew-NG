...
%find cost func for movies where rating is present

h = X*Theta';
for i =1: size(X,1)
  for j=1 :size(Theta,1)
    if (R(i,j)==1)
      temp= temp+ (h(i,j)- Y(i,j))^2;
    endif
  endfor
endfor
J= temp/2;

% regularixed for the same
for i= 1: size(X,1)
  for j= 1: size(X,2)
    a=a+ X(i,j)^2;
  endfor
...
J= J+ (lambda*a)/2 + (lambda*b)/2;

%find grad for x and theta
for i= 1:size(R,1)
  for j = 1:size(R,2)
    for k = 1 :size(X,2)
      if (R(i,j)==1)
